...
# Assemble data for Shiny app for later use

require(tidyverse)
newdata <- readRDS("result_2016-10-08.rds") # file too large to store in github
headers <- read.csv("list_labels.csv", h = T)
headers <- select(.data = headers, pts, days, psi1, p, phi)

# function that isolates values from simulation years for a particular combination of simulation parameters and calculates
# confidence limits, mean and median
...
# Run summarize_sims() for all possible combination of parameters
...
#join with the header columns
timeSeriesData <- data.frame(headers,res80,res90,res95)
rownames(timeSeriesData) <- NULL

#save as an RDS object ready use with the Shiny app
saveRDS("data",timeSeriesData)
