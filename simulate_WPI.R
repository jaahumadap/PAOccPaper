...
simulateCommunity <- function(nspecies, points, years, nsim, phi, gamma,p){
        ...
        #set.seed(234)
        #phi <- runif(nspecies,0.925,0.975)
        ...
        for (i in 1:nspecies) {
                ...
                res[,,i] <- simulatetrend(points = points, phi = phi, gamma = gamma, years = years, nsim = nsim, p = p)
...
# for final analysis
spDec <- simulateCommunity(nspecies = 16,points = 60,years = 16,nsim = 100,phi = 0.9,gamma = 0)
control <- simulateCommunity(nspecies = 16,points = 60,years = 16,nsim = 100,phi = 0.9,gamma = 0.1)
